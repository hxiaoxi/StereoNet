...
root_path = "/home/huangjx/Documents/StereoNet"
...
if args.loadmodel is not None and os.path.exists(checkpoint_path):
    ...
    scheduler = torch.optim.lr_scheduler.ExponentialLR(
        optimizer, gamma=0.9, last_epoch=epoch_start)
else:
...
def main():
    ...
    for epoch in range(epoch_start, args.epochs+1):
        ...
        print('epoch %d total training loss = %.3f' %
              (epoch, avg_train_loss/len(TrainImgLoader)))
...
from disparity.models.loss import My_loss  # 源代码的loss模块丢失，可能闭源了
...
def main():
    ...
    cfg.warmup = getattr(cfg, 'warmup', True) if not args.debug else False # get attribute？
...
def main_worker(gpu, ngpus_per_node, args, cfg, exp):
    ...
    train_left_img, train_right_img, train_left_disp, train_left_norm, test_left_img, test_right_img, test_left_disp, test_left_norm = ls_2015.dataloader(
        args.datapath)
    train_left_img12, train_right_img12, train_left_disp12, train_left_norm12, test_left_img12, test_right_img12, test_left_disp12, test_left_norm12 = ls_2012.dataloader(
        args.datapath12)
    ...
    ImageFloader = DA.myImageFloder(train_left_img+train_left_img12, train_right_img+train_right_img12,
                                    train_left_disp+train_left_disp12, train_left_norm+train_left_norm12, True)
